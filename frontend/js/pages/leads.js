...
window.loadLeads = async function() {
    ...
    try {
        let url = `${API_BASE}/companies/${companyId}/leads?page=1&per_page=50`;
        ...
        if (data.data && data.data.length > 0) {
            table.innerHTML = `
                <table class="table-advanced">
                    ...
                    <tbody>
                        ${data.data.map(lead => `
                            <tr>
                                <td><strong>${escapeHtml((lead.first_name && lead.last_name) ? `${lead.first_name} ${lead.last_name}` : lead.lead_name || '-')}</strong></td>
                                <td>${escapeHtml(lead.company_name || '-')}</td>
                                <td>${escapeHtml(lead.email || '-')}</td>
                                <td>${escapeHtml(lead.phone || '-')}</td>
                                <td>${escapeHtml(lead.source || '-')}</td>
                                <td><span class="activity-badge badge-${lead.status || 'new'}">${escapeHtml((lead.status || 'new').toUpperCase())}</span></td>
                                <td><span class="badge badge-${lead.priority || 'medium'}">${escapeHtml((lead.priority || 'medium').toUpperCase())}</span></td>
                                <td>${lead.lead_score !== null && lead.lead_score !== undefined ? lead.lead_score : '-'}</td>
                                <td>${lead.estimated_value ? `₹${parseFloat(lead.estimated_value).toLocaleString()}` : '-'}</td>
                                <td>
                                    <button class="btn-icon btn-edit" onclick="editLead(${lead.id})" title="Edit">
            ...
            table.innerHTML = '<div class="empty-state"><h3>No leads found</h3><p>Create your first lead!</p></div>';
        }
    } catch (error) {
