...
window.loadContacts = async function() {
    ...
    try {
        let url = `${API_BASE}/companies/${companyId}/contacts?page=1&per_page=100`;
        ...
        if (!response.ok) {
            ...
            console.error('Error loading contacts:', response.status, errorData);
            const table = document.getElementById('contactsTable');
            if (table) {
            ...
            window.contactsTable = new DataTable('contactsTable', {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        filterable: true,
                        render: (value, row) => {
                            return row.account ? escapeHtml(row.account.name) : '-';
                        }
                    },
                    {
                        ...
                        align: 'center',
                        render: (value) => {
                            return value ? '<span class="status-badge status-active">Primary</span>' : '-';
                        }
                    },
                    {
                        key: 'actions',
                        ...
                        render: (value, row) => {
                            return `
                                <button class="btn-icon btn-edit" onclick="editContact(${row.id})" title="Edit">
                                ...
                                <button class="btn-icon btn-delete" onclick="deleteContact(${row.id})" title="Delete">
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
