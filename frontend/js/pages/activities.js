...
window.initActivities = function() {
    ...
    if (!authToken || !companyId) {
        ...
        if (list) {
            list.innerHTML = '<div class="empty-state"><h3>Please login and select a company</h3></div>';
        }
        return;
    }
...
window.loadActivities = async function() {
    ...
    try {
        let url = `${API_BASE}/companies/${companyId}/activities?page=1&per_page=50`;
        ...
        if (data.data && data.data.length > 0) {
                ...
                <div class="activity-item-advanced">
                    <div class="activity-info">
                        ...
                        <p class="activity-meta">
                            <span class="badge badge-secondary">${escapeHtml(activity.activity_type || '-')}</span>
                            <span>•</span>
                        ...
                        ${activity.duration ? `<p class="activity-duration">Duration: ${activity.duration} minutes</p>` : ''}
                    </div>
                    <div class="activity-actions">
                        ${activity.outcome ? `<span class="activity-badge badge-${activity.outcome}">${escapeHtml(activity.outcome.replace('_', ' ').toUpperCase())}</span>` : ''}
                        <button class="btn-icon btn-edit" onclick="editActivity(${activity.id})" title="Edit">
                        ...
                        <button class="btn-icon btn-delete" onclick="deleteActivity(${activity.id})" title="Delete">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            ...
            list.innerHTML = '<div class="empty-state"><h3>No activities found</h3><p>Log your first activity!</p></div>';
        }
    } catch (error) {
        console.error('Error loading activities:', error);
        const list = document.getElementById('activitiesList');
        if (list) {
            list.innerHTML = '<div class="empty-state"><h3>Error loading activities</h3><p>Please try again</p></div>';
        }
    }
};
